...
#define REG_BUFF_RESULT unsigned int 

enum REG_BUFF_RESULT_ENUM {
    REG_BUFF_RESULT_SUCC
};
...
enum CONNECT_PEER_RESULT_ENUM {
    ...
    //空guid
    CONNECT_PEER_NULL_GUID,
    //已经连接
    CONNECT_PEER_TARGET_CONNECT_ALREADY,
    //注册回调失败
    CONNECT_PEER_REG_ERROR,
    //打开Nat功能失败
    CONNECT_PEER_OPEN_NAT_ERROR
...
#define GET_PEER_IP_RESULT unsigned int 

enum GET_PEER_IP_RESULT_ENUM {
    //
    GET_PEER_IP_SUCC,
    //
    GET_PEER_IP_NOT_INIT_NETWORK,
    //
//    GET_PEER_IP_ERROR_NOTICE,
    //
    GET_PEER_IP_TARGET_NOT_CONNECT
};
#endif	/* ERRERNUMBER_H */
